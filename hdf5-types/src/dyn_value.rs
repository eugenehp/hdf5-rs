    ...
    pub fn iter(&self) -> impl Iterator<Item = (&str, DynValue<'_>)> {
        self.tp.fields.iter().map(move |field| {
            (
                ...
                DynValue::new(
                    &field.ty,
                    &self.buf[field.offset..(field.offset + field.ty.size())],
                ),
            )
        })
    }
...
impl OwnedDynValue {
    pub fn new<T: H5Type>(value: T) -> Self {
        ...
        mem::forget(value);
        Self {
            tp: T::type_descriptor(),
            buf: buf.to_owned().into_boxed_slice(),
        }
    }
...
mod tests {
    ...
    fn test_dyn_value_from() {
        ...
        let s = big_struct_2();
        assert_eq!(
            OwnedDynValue::from(s.clone()),
            OwnedDynValue::new(s.clone())
        );
    }
