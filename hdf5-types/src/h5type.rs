...
impl TypeDescriptor {
    ...
    fn c_alignment(&self) -> usize {
        match *self {
            Self::Compound(ref compound) => compound
                .fields
                .iter()
                .map(|f| f.ty.c_alignment())
                .max()
                .unwrap_or(1),
...
macro_rules! impl_tuple {
    ...
    (@parse_fields [$($s:ident)*] $origin:ident $fields:ident | $t:ty $(,$tt:ty)*) => (
        {
            // Compute the field offset from an uninitialized (but valid,
            // non-null) tuple pointer without materializing a reference.
            let base: *const Self = $origin.as_ptr();
            let (.., ref f, $(impl_tuple!(@second $tt _),)*) = unsafe { &*base };
            let offset = (f as *const _ as usize) - (base as usize);
    ...
    ($t:ident, $($tt:ident),*) => (
        ...
        {
            fn type_descriptor() -> TypeDescriptor {
                let origin = mem::MaybeUninit::<Self>::uninit();
